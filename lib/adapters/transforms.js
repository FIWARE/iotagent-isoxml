...
const FMIS = {
    ...
    addId(attr, entity, type, attributeName = 'A') {
        const match = entity.id.match(idRegex);
        attr[attributeName] = String(type) + (match ? match[0] : this.idIndex++);
    },
...
const MICS = {
    ...
    addProperty(entity, from, to, type = 'Property', normalized = true) {
        if (Object.prototype.hasOwnProperty.call(entity, from)) {
            const value = getValue(entity[from]).replace(/[<>"'=;()]/g, ' ');
