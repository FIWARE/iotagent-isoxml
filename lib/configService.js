...
function processEnvironmentVariables() {
    ...
    if (config.iota.types) {
        ...
        Object.keys(config.iota.types).forEach((key) => {
            if (config.iota.types[key].static_attributes) {
                if (
                    !_.any(config.iota.types[key].static_attributes, function (item) {
                        return _.isEqual(item, isoxml[key]);
                    })
                ) {
                    config.iota.types[key].static_attributes.push(isoxml[key]);
                }
            } else {
                config.iota.types[key].static_attributes = [isoxml[key]];
            }
        });
    }
}
