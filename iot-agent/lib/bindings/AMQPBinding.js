...
function commandHandler(device, attributes, callback) {
  config
    .getLogger()
    .debug(context, "Handling AMQP command for device [%s]", device.id);
...
function queueListener(msg) {
  ...
  commons.amqpMessageHandler(
    msg.fields.routingKey.replace(/\./g, "/"),
...
function start(callback) {
  ...
  if (
    config.getConfig() &&
    config.getConfig().amqp &&
    config.getConfig().amqp.retrytime
  ) {
